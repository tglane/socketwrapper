...
- Other:
    ...
    // Check if SO_ERROR is set for the socket and return the error
...
- Constructor:
    ...
    tcp_connection(endpoint<IP_VER> endpoint);
    ```
- Config:
    ```cpp
    // Connect a default constructed socket to a given endpoint
    void connect(endpoint<IP_VER> endpoint);

    template <typename callback_type>
    void async_connect(endpoint<IP_VER> endpoint, callback_type&& callback);

    op_awaitable<void, net::tcp_connection::no_op_operation> co_connect(endpoint<IP_VER> endpoint);

    std::future<void> promised_connect(endpoint<IP_VER> endpoint);
    ```
- Reading:
    ```cpp
    // Read as much bytes as fit into buffer and block until the read operation finishes or the delay is over.
    std::optional<size_t> read(net::span<T> buffer, const std::optional<std::reference_wrapper<const std::chrono::duration<int64_t, std::milli>>> timeout = std::nullopt) const;
    ...
    net::op_awaitable<size_t, net::tcp_connection::stream_read_operation<T>> co_read(net::span<T> buffer) const;
...
- Sending:
    ...
    std::optional<size_t> write(net::span<T> buffer, const std::optional<std::reference_wrapper<const std::chrono::duration<int64_t, std::milli>>> timeout = std::nullopt) const;
    ...
    void async_write(net::span<T> buffer, CALLBACK_TYPE&& callback) const;

    // Immediately returns an awaitable that can be co_awaited in a C++20 coroutine
    // Only available when compiling with C++20 or higher
    net::op_awaitable<size_t, net::tcp_connection::stream_write_operation<T>> co_write(net::span<T> buffer) const;

    // Immediately return and get a future to get the number of elements written at a later point in time
    std::future<size_t> promised_write(net::span<T> buffer) const;
    ```
...
- Config:
    ...
    void listen(const endpoint<IP_VER>& endpoint, const size_t backlog = 5);
    ```
- Accepting:
    ```cpp
    ...
    std::optional<tcp_connection<IP_VER>> accept(const std::optional<std::reference_wrapper<const std::chrono::duration<int64_t, std::milli>>> timeout = std::nullopt) const;
    ...
    // Immediately returns an awaitable that can be co_awaited in a C++20 coroutine
    // Only available when compiling with C++20 or higher
    net::op_awaitable<net::tcp_connection<IP_VER>, net::tcp_acceptor::stream_accept_operation> co_accept() const;
...
- Reading:
    ...
    std::pair<size_t, std::optional<endpoint<IP_VER>>> read(span<T> buffer, const std::optional<std::reference_wrapper<const std::chrono::duration<int64_t, std::milli>>> timeout = std::nullopt) const;
    ...
    // Immediately returns an awaitable that can be co_awaited in a C++20 coroutine
    // Only available when compiling with C++20 or higher
    net::op_awaitable<std::pair<size_t, std::optional<endpoint<IP_VER>>>, net::udp_socket::dgram_read_operation<T>> co_read(span<T> buffer) const;
...
- Writing:
    ```cpp
    std::optional<size_t> write(const endpoint<IP_VER>& endpoint_to, span<T> buffer, const std::optional<std::reference_wrapper<const std::chrono::duration<int64_t, std::milli>>> timeout = std::nullopt) const;
    ...
    void async_write(const endpoint<IP_VER>& endpoint_to, span<T> buffer, CALLBACK_TYPE&& callback) const;

    // Immediately returns an awaitable that can be co_awaited in a C++20 coroutine
    // Only available when compiling with C++20 or higher
    net::op_awaitable<size_t, net::udp_socket::dgram_write_operation<T>> co_write(const endpoint<IP_VER>& endpoint_to, span<T> buffer) const;

    // Immediately return and get a future to get the number of elements written at a later point in time
    std::future<size_t> promised_write(const endpoint<IP_VER>& endpoint_to, span<T> buffer) const;
    ```
- Shorthand identifier:
    ```cpp
