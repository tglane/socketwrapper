...
int main(int argc, char** argv)
{
    ...
    if(strcmp(argv[1], "r") == 0)
    {
        ...
        net::udp_socket sock {net::ip_version::v4, "0.0.0.0", 4433};
        auto buffer = sock.read<char>(512);
        std::cout << std::string_view {buffer.data(), buffer.size() } << '\n';

        // net::tcp_acceptor acceptor {net::ip_version::v4, "0.0.0.0", 4433};
        // // net::tcp_acceptor acceptor {net::ip_version::v6, "::", 8800};
    ...
    {
        std::cout << "--- Sender ---\n";
        net::udp_socket sock {net::ip_version::v4};
        std::vector<char> buffer {'h', 'a', 'l', 'l', 'o'};
        sock.send("127.0.0.1", 4433, buffer);

        // net::tcp_connection sock {net::ip_version::v4, "127.0.0.1", 4433};
        // std::cout << "Connected\n";
